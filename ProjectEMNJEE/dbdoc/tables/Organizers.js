...
var relations = [
	...
	target: { id: classes.ORGANIZERS.id },
	vertices: [],
	smooth: false,
	attrs: {
	...
	labels: [
	{ position: 60, attrs: { text: { text: 'SQL141020174648962' } }}
	]
}),	
								];
...
_.each(relations, function(r) { r.toBack(); });
 ...
 function saveGraph(){
	if(typeof(Storage)!=="undefined")
  	{
		localStorage.jsonOrganizersGraph=(JSON.stringify(graph.toJSON()));
  	}
	else
  	{
 ...
 function loadGraph(){
	if(typeof(Storage)!=="undefined")
  	{
		 graph.fromJSON(JSON.parse(localStorage.jsonOrganizersGraph));
  	}
	else
  	{
  		document.getElementById("error").innerHTML="Sorry, your browser does not support web storage...";
